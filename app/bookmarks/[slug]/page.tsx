...
import { getAllBookmarks, getBookmarkBySlug } from "@/lib/content";
import { formatDate } from "@/lib/utils";

export function generateStaticParams(): Array<{ slug: string }> {
  return getAllBookmarks().map((entry) => ({ slug: entry.frontmatter.slug }));
}
...
export default async function BookmarkDetailPage({
  ...
  const entry = getBookmarkBySlug(slug);
  ...
  return (
    <article className="rounded-2xl border border-border bg-surface p-8 shadow-soft">
      <p className="text-sm uppercase tracking-wide text-accent">Bookmark</p>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <CopyLinkButton path={`/bookmarks/${entry.frontmatter.slug}`} />
        <a
          className="rounded-lg bg-text px-3 py-2 text-sm text-white transition hover:bg-accent"
