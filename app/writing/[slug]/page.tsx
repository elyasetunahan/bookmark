...
export function generateStaticParams(): Array<{ slug: string }> {
  return getAllWritings().map((entry) => ({ slug: entry.frontmatter.slug }));
}
...
export default async function WritingDetailPage({
  ...
  return (
    <article className="rounded-2xl border border-border bg-surface p-8 shadow-soft">
      <p className="text-sm uppercase tracking-wide text-accent">Writing</p>
      ...
      <div className="mt-5 flex items-center gap-3">
        <CopyLinkButton path={`/writing/${entry.frontmatter.slug}`} />
      </div>
